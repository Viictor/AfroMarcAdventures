 ...
 * @author Victor
 */
public class DeadlyBoundingRect extends BoundingShape {
    ...
    public DeadlyBoundingRect(float x, float y, float width, float height) {
        this.x = x + width / 2;
        this.y = y + height / 2;
        this.width = width;
        this.height = height;
    ...
    public boolean checkCollision(AABoundingRect box) {
        boolean dead =  !(box.x - box.width / 2 > this.x + width / 2 || box.x + box.width / 2 < this.x - width / 2
     ...
     *
     * @return -1 if its off the lowest margin, 1 if its off any of the other
     * margins, 0 if it's not.
     */
    @Override
